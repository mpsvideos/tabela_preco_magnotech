...
Versão: 1.0 - layout Tkinter + Custom Tkinter.
Versão: 1.1 - Automação de tarefas, adicionado a função backup do banco de dados.
Sempre que rodar o programa, será feita uma cópia de segurança do banco de dados em um diretório diferente.
O programa passa a ser Cadastro de Produtos.
A janela passa de 900x510 para maximizada.
Acrescentado o campo quantidade de produtos.
No campo de pesquisa o processo é automatizado por filtragem autocomplete, a medida que o usuário digita,
é exibido o resultado da pesquisa.
"""
        ...
        try:
            if entry_marca.get() == '' or entry_modelo.get() == '' or entry_valor.get() == '' or entry_quantidade.get() == '':
                messagebox.showinfo('Atenção!',
                                    'Se você não preencher os campos\n[produto, modelo, valor e quantidade]\n'
                                    'não será possível cadastrar!\n'
                ...
                cursor.execute(f"""
                    ...
                    VALUES('{marca}', '{modelo}', '{valor_formatado}', '{quantidade}') 
                """)
    ...
    def salvar_dados():
        with conn:
            try:
                ...
                cursor.execute(f"""
                                    UPDATE produtos SET marca='{marca}', modelo='{modelo}', valor='{valor_tratado}',
                ...
                # rever essa condição..................................................................
                # aqui exibe a mensagem que o estoque está baixo
                lista = mostrar_dados()

                for item in lista:
                    if item[4] == 0:
                        messagebox.showinfo('Atenção!', 'O estoque desse produto está zerado!!!')
                        break
                    elif 0 < item[4] <= 1:
                        messagebox.showinfo('Atenção!', 'Tem apenas um produto em estoque.')
                        break
                    elif item[4] < 0:
                        return messagebox.showerror('Erro!', 'Não é possível número negativo.')
...
imagem = imagem.resize((350, 200))
foto = ImageTk.PhotoImage(imagem)

label_imagem = Label(frame_cima, image=foto, bg='#d9d9d9')
label_imagem.place(x=950, y=30)
